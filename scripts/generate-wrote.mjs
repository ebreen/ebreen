...
function renderCard(posts, theme) {
  const rows = posts
    ...
    <circle cx="54" cy="${cy}" r="2.5" fill="${theme.accent}"/>
    <text x="72" y="${y}" class="sans" font-size="16" font-weight="600" fill="${theme.text}">${escapeXml(post.title)}</text>
    <text x="792" y="${y}" class="mono" font-size="12" fill="${theme.muted}" text-anchor="end">${escapeXml(post.displayDate)}</text>
  </g>`;
    })
    .join("\n");

  const footerDelay = posts.length ? DELAY[Math.min(posts.length, 3) - 1] : "";
  const footerClass = footerDelay ? `rise ${footerDelay}` : "rise";

  return `<svg xmlns="http://www.w3.org/2000/svg" width="840" height="${HEIGHT}" viewBox="0 0 840 ${HEIGHT}" role="img">
  <defs><style>
${STYLE}
</style></defs>
  <rect x="0.5" y="0.5" width="839" height="${HEIGHT - 1}" rx="20" fill="${theme.bg}" stroke="${theme.stroke}"/>
  <g class="rise">
...
${rows ? rows + "\n" : ""}  <g class="${footerClass}">
    <text x="48" y="${FOOTER_Y}" class="mono" font-size="12" fill="${theme.muted}">all notes at eirikbreen.com/blog</text>
  </g>
</svg>
`;
}
